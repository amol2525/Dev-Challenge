...
public class AccountsService {
	...
	public boolean transferAmount(final String fromAccountId, final String toAccountId, BigDecimal amount)
		...
		if (isAccountIdNotEmpty(fromAccountId) && isAccountIdNotEmpty(toAccountId)  && isTransferAmountValid(amount)) {
	...
	private boolean isAccountIdNotEmpty(String accountId) throws InvalidAccountException {
		if(StringUtils.isEmpty(accountId))
				throw new InvalidAccountException("AccountId can not be null or empty");
		return true;
	}

	private boolean isTransferAmountValid(BigDecimal amount) throws InvalidTransferAmountException {
		if(amount==null || amount.compareTo(BigDecimal.ONE)<-1)
			throw new InvalidTransferAmountException("Invalid tranfer amount!! Amount cannot be null,zero or negative");
		return true;
	}
	...
	private boolean transferMoney(final Account fromAcct, final Account toAcct, final BigDecimal amount)
		...
		if (fromAcct.getBalance().compareTo(amount) < 0)
			...
			notificationService.notifyAboutTransfer(toAcct,
					"Account " + toAcct.getAccountId() + " credited with amount " + amount);
